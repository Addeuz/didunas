...
<script lang="ts">
	...
	// 	try {
	// 		const res = await fetch(`/api/auth/login/${name}`, {
	// 			method: 'POST',
	// 			body: JSON.stringify({
	// 				name: 'janne',
	// 				lastname: 'andersson'
...
<!-- {#await result then testUser}
	<p class="text-blue-400">{testUser?.user.username}</p>
{/await} -->
...
>
	...
	>
		<img
			...
			src="/patternRecognitionTask.png"
			alt="Pattern recognition task"
		/>
